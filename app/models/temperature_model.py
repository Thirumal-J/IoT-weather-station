...
def getDbConnection():
    try:
        ...
        staticVar.connection = psycopg2.connect(user = appConf.temperatureDBConfig["user"],
                                        password = appConf.temperatureDBConfig["password"],
                                        host = appConf.temperatureDBConfig["host"],
                                        port = appConf.temperatureDBConfig["port"],
                                        database = appConf.temperatureDBConfig["database"])
...
def createTemperatureTable():
    try:        
        if (isTableExist(appConf.temperatureTableName)):
            return {"TableName":appConf.temperatureTableName,"queryStatusUpdate": staticVar.DBQueryStatus}
        else:
            create_table_query = f'CREATE TABLE IF NOT EXISTS public."{appConf.temperatureTableName}"(fetchedTime TIMESTAMP NOT NULL, fetchedData jsonb NOT NULL, PRIMARY KEY (fetchedTime));'
            alter_table_query = f'ALTER TABLE public."{appConf.temperatureTableName}" OWNER to {appConf.temperatureDBConfig["user"]};'
...
def fetchDataFromTable(tableName,timeperiod):
    try:
        ...
        staticVar.connection.commit()
        allTemperatureData = staticVar.cursor.fetchall()
        results = {}
        count = 0
