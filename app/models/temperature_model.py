...
def getDbConnection():
    try:
        con.connection  = lite.connect("temperaturedb.db", check_same_thread=False)
...
def dropTable(tableName):
    try:
        if(isTableExist(tableName)):
            ...
            queryStatusUpdate(constant.SUCESS_STATUS_CODE,constant.SUCESS_STATUS_MESSAGE,constant.SQLITE3_DROP_TABLE_SUCCESS_MESSAGE)
        else:
...
def createTemperatureTable(tableName):
    try:        
        if (isTableExist(tableName)):
            ...
            #alter_table_query = 'ALTER TABLE public."{temperatureTableName}" OWNER to postgres;'
...
def fetchDataFromTable(tableName,timeperiod):
    try:
        ...
        if(timeperiod.lower()=='last24hours'):
            con.days_to_subtract = 1
        elif(timeperiod.lower()=='lastweek'):
     ...
     #   fetch_query = "SELECT fetcheddata FROM "+tableName+" WHERE fetchedTime >='"+ con.dataFrom+"' AND fetchedtime < '"+ con.dataTill+"' order by fetchedtime asc;"
        print("*********",fetch_query)
        con.cursor.execute(fetch_query)
        con.connection.commit()
        alltemperatureData = con.cursor.fetchall()
        results = {}
        count = 0
        for row in alltemperatureData:
...
def closeConnection():
    try:
        if(con.isDBConnected):
            ...
            print(f"{constant.CLOSE_CONNECTION_SUCCESS_MESSAGE} >>> DB Connection Status >>> {con.isDBConnected}")
            return constant.CLOSE_CONNECTION_SUCCESS_MESSAGE # Should be logged
    except (Exception, lite.DatabaseError) as error :
        print (f"{constant.CLOSE_CONNECTION_FAILURE_MESSAGE} >>> {error}") # Should be logged
        return (f"{constant.CLOSE_CONNECTION_FAILURE_MESSAGE} >>> {error}") # Should be logged